...
test('Codex executor accepts documented JSONL event stream', async () => {
  ...
  const executor = createCodexExecutor({ spawnImpl: fakeSpawn(stdout), env: { PATH: '/bin', HOME: '/tmp' } });
...
test('Codex executor preserves the historical single-line JSON payload shape', async () => {
  ...
  const executor = createCodexExecutor({ spawnImpl: fakeSpawn(JSON.stringify(payload)), env: { PATH: '/bin', HOME: '/tmp' } });
  const result = await executor.execute(invocation());
