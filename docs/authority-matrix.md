...
| `maws-vnext:jev-decision-engine` | config surface | n/a | runtime-implemented | decision-threshold-policy + decision-receipt schemas + `runtime/decision-engine/jev/` + `runtime/activation/live-activation.mjs` | fixture-mode typed decisions plus a three-lane activation harness exist; the canonical live path is now Jev via the OpenRouter Decisions API (mode `openrouter`, `OPENROUTER_API_KEY` only; receipt mode enum additively extended), with direct TypeSafe preserved as optional compat provider; live activation remains unproven until credential-backed evidence records a PASS |
| `maws-vnext:executor-transports` | repo surface | n/a | runtime-implemented | `runtime/executors/` + `runtime/transports/local-process.mjs` + `runtime/activation/live-activation.mjs` + `tests/vnext/` | Codex executor (`exec_codex_chatgpt`) runs on the ChatGPT Codex plan (chatgpt_oauth / chatgpt_plan; no OpenRouter coupling) and is LIVE_VERIFIED_LOCAL including a bounded write tracer with deterministic byte verification; the direct OpenRouter model executor and the Jev-via-OpenRouter Decisions lane are implemented (verified API contract) but their live PASS evidence is credential-blocked (`OPENROUTER_API_KEY` absent locally); JSONL `turn.failed` fail-closed classification and env secret boundaries are test-enforced; provider adapters stay pure |
