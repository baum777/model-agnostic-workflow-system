...
| `maws-vnext:jev-decision-engine` | config surface | n/a | runtime-implemented | decision-threshold-policy + decision-receipt schemas + `runtime/decision-engine/jev/` + `runtime/activation/live-activation.mjs` | fixture-mode typed decisions and a bounded live-activation harness exist; live TypeSafe activation remains unproven until credential-backed evidence records a PASS |
| `maws-vnext:executor-transports` | repo surface | n/a | runtime-implemented | `runtime/executors/` + `runtime/transports/local-process.mjs` + `runtime/activation/live-activation.mjs` + `tests/vnext/` | Codex and OpenRouter transports plus a bounded activation runner are implemented; Codex inference routes through an isolated OpenRouter-bound CODEX_HOME with explicit model binding (missing binding fails closed before spawn) and live-observed `turn.failed` event classification; transport wiring is live-verified against the real OpenRouter endpoint via a synthetic-credential 401 probe (codex-cli 0.157.0); credential-backed inference PASS evidence remains required; provider adapters stay pure |
